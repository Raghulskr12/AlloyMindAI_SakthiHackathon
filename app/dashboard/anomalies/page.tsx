...
export default function AnomaliesPage() {
  ...
  const rootCauseStats = (() => {
    const counts: { [key: string]: number } = anomalies.reduce((acc: { [key: string]: number }, anomaly) => {
  ...
  const updateAnomalyStatus = (anomalyId: string, newStatus: 'active' | 'investigating' | 'resolved') => {
    setAnomalies(prev =>
      prev.map(anomaly =>
    ...
    const statusText: { [key in 'active' | 'investigating' | 'resolved']: string } = {
      ...
      <div className="flex-1 space-y-6 p-6 bg-slate-900 min-h-screen">
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center space-x-4">
            <SidebarTrigger />
            <div>
              <h1 className="text-3xl font-bold text-white">Anomaly Center</h1>
              <p className="text-slate-400">Live alerts and anomaly monitoring</p>
            </div>
          </div>
        </div>
