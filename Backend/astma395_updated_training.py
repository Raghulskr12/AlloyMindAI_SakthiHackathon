#!/usr/bin/env python3
"""
ASTMA395 Element Configuration Training Pipeline - Updated Targets
Specialized training pipeline for ASTMA395 (Ferritic Ductile Iron) grade with updated target configurations:
- Target C: 3.6%, Si: 3.0%, Mn: 0.725%, P: 0.0%, S: 0.004%, Cu: 0.175%, Mg: 0.04%
"""
...
def update_target_specifications(df):
    ...
    print("=" * 60)
    print("UPDATING TARGET SPECIFICATIONS FOR ASTMA395")
    print("=" * 60)
    ...
    for element in elements:
        ...
        if target_col in df.columns:
            ...
            if element in ['Mn', 'P']:  # Highlight major changes
                change += " ⚠️"
    ...
    print("Key Changes:")
    print("  🔥 Mn: 0.225% → 0.725% (3.2x increase)")
    print("  🎯 P: 0.020% → 0.000% (zero phosphorus)")
    return df

def analyze_dataset(df):
    """Analyze the ASTMA395 dataset structure."""
    print("\n" + "=" * 60)
    print("ASTMA395 DATASET ANALYSIS - UPDATED TARGETS")
    print("=" * 60)
...
def preprocess_data(df, feature_columns, target_columns):
    ...
    for element in elements:
        ...
        if current_col in X.columns and target_col in X.columns:
            ...
            # Ratio of target to current (handle zero P target)
            ratio_col = f'{element}_Ratio'
            if element == 'P':  # Special handling for zero P target
                X[ratio_col] = 0.0  # Zero ratio for zero target
            else:
                X[ratio_col] = X[target_col] / (X[current_col] + 1e-8)
            engineered_features.append(ratio_col)
            
            # Relative change percentage (handle zero P target)
            pct_change_col = f'{element}_PctChange'
            if element == 'P':  # Special handling for zero P target
                X[pct_change_col] = -100.0 * X[current_col] / (X[current_col] + 1e-8)  # Negative change to zero
            else:
...
def train_lightgbm_models(X_scaled, y, test_size=0.2, random_state=42):
    """Train LightGBM models for ASTMA395 Element_Config prediction."""
    print("\n" + "=" * 60)
    print("LIGHTGBM MODEL TRAINING - ASTMA395 UPDATED")
    print("=" * 60)
    ...
    # LightGBM parameters optimized for ASTMA395
    lgb_params = {
...
def train_catboost_models(X, y, categorical_features, test_size=0.2, random_state=42):
    """Train CatBoost models for ASTMA395 Element_Config prediction."""
    print("\n" + "=" * 60)
    print("CATBOOST MODEL TRAINING - ASTMA395 UPDATED")
    print("=" * 60)
    ...
    # CatBoost parameters optimized for ASTMA395
    catboost_params = {
        'iterations': 1000,
        'learning_rate': 0.05,
        'depth': 6,
...
def evaluate_ensemble(lgbm_models, catboost_models, X_test_scaled, X_test_raw, y_test, scaler):
    """Evaluate the ASTMA395 updated ensemble model."""
    print("\n" + "=" * 60)
    print("ASTMA395 UPDATED ENSEMBLE MODEL EVALUATION")
    print("=" * 60)
    ...
    print(f"\nASTMA395 Updated Ensemble Average Performance:")
...
def save_models(ensemble_model, lgbm_training_info, catboost_training_info, ensemble_results, label_encoders):
    """Save all ASTMA395 updated models and training information."""
    print("\n" + "=" * 60)
    print("SAVING ASTMA395 UPDATED MODELS")
    print("=" * 60)
    ...
    joblib.dump(ensemble_package, 'astma395_updated_ensemble_model.pkl')
    print("✓ Saved ASTMA395 updated ensemble model: astma395_updated_ensemble_model.pkl")
    ...
    training_info = {
        ...
        'key_changes': {
            'manganese': 'Increased from 0.225% to 0.725% (3.2x)',
            'phosphorus': 'Reduced from 0.020% to 0.000% (zero P)'
        },
    ...
    with open('astma395_updated_training_info.json', 'w') as f:
        json.dump(training_info, f, indent=2)
    print("✓ Saved ASTMA395 updated training info: astma395_updated_training_info.json")

def main():
    """Main training pipeline for ASTMA395 with updated targets."""
    print("ASTMA395 Element Configuration Ensemble Training Pipeline - UPDATED TARGETS")
    print("=" * 75)
    print("Updated Target Specifications:")
    print("  C: 3.6%, Si: 3.0%, Mn: 0.725% (3x higher), P: 0.0% (zero)")
    print("  S: 0.004%, Cu: 0.175%, Mg: 0.04%")
    print("=" * 75)
    
    # Load dataset
    print("Loading ASTMA395 dataset with ElementConfig...")
    df = pd.read_csv('ASTMA395_Dataset_with_ElementConfig/ASTMA395_with_element_config.csv')
    ...
    # Final summary
    print("\n" + "=" * 75)
    print("ASTMA395 UPDATED TRAINING COMPLETE")
    print("=" * 75)
    
    avg_r2 = np.mean([results['r2'] for results in ensemble_results.values()])
    print(f"ASTMA395 Updated Ensemble Model Performance: R² = {avg_r2:.4f}")
    print(f"Element Configurations Predicted: {len(target_columns)}")
    print(f"Features Used: {len(feature_columns)}")
    
    print("\nFiles created:")
    print("  - astma395_updated_ensemble_model.pkl")
    print("  - astma395_updated_training_info.json")
    
    print("\nUpdated Target Specifications:")
    for elem, target in ensemble_model.target_specs.items():
        if elem == 'Mn':
            print(f"  {elem}: {target}% 🔥 (3x increase)")
        elif elem == 'P':
            print(f"  {elem}: {target}% 🎯 (zero phosphorus)")
        else:
            print(f"  {elem}: {target}%")
